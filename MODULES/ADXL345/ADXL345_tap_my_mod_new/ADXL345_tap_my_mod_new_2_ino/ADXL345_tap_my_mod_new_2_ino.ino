...
void setup(void) 
{
  ...
  Activites activ = accelerometer.readActivites();
   ...
   if(activ.isTapOnX){
      menusetup = menusetup + 1;
      
    }else if(menusetup > 1){
      menusetup = 0;
    }
    
   if(setmenu == 0 && menusetup == 0){
     if(activ.isDoubleTap){
       setmenu = 1;
     }
     
   }else if(setmenu == 0 && menusetup == 1){
     if(activ.isDoubleTap){
       setmenu !=0;
       Serial.println(m);
     }
  ...
  while(setmenu == 0){
    ...
    switch(menusetup){
      case 0:
        Serial.println("Double Tap for Skip");
        delay(1000);
        break;
      case 1:
        Serial.println("Double Tap for Set M_Value");
        delay(1000);
        break;
      default:
        menusetup = 0;
...
void loop(void) 
{
  ...
  if (activ.isTapOnX){
    Serial.println("Move list down");
  } else
  if (activ.isTapOnY){
    Serial.println("Switch list into submenu");
  } else
  if (activ.isActivityOnZ){
    if (activ.isDoubleTap){
