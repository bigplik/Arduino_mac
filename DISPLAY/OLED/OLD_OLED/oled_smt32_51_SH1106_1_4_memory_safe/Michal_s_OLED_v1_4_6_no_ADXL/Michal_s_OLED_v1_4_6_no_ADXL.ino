...
void logo3(void) {
    ...
    u8g.drawStr( 27, 60, "v.1.4.4 ADX-less");
    u8g.drawRFrame(16,20,97,30, a+1);
}
...
void screens() {
  ...
  uint8_t a;
    //u8g.setFont(u8g_font_profont17);//10-17
          ...
          //delay(500);
          //u8g.drawStr( 0, 0, "drawRFrame/Box");
          u8g.drawRFrame(58, 32,70,30, a+1); //1-ys, 2-, 3szer, 4-wysokosc famki
...
void setup(void) {
  ...
  digitalWrite(interruptPin, HIGH); //podciągnięcie pinu w stan wysoki
  attachInterrupt(
...
bat += analogRead(A0);                          //sumowanie kolejnych pomiarów
...
void oledPower(){
  ...
  while (millis() > 5000) {
    ...
    if (PPO2 < 0.25 || PPO22 < 0.25 || PPO23 < 0.25 &&
    PPO2 > 0.20 || PPO22 > 0.20 || PPO23 > 0.20) {
      u8g.sleepOff();
    }else
    {
      u8g.sleepOn();
